...
fungi_markers=c("ITS","18S","ITS","18S","ITS")
...
arbre_reseau <- function (fungi,marker,site){
    ...
    if (length(grep("outgroup", tree$tip.label))>1) {tree <- root(tree, node = getMRCA(tip=tree$tip.label[grep("outgroup", tree$tip.label)], tree))
    }else{tree <- root(tree, outgroup = tree$tip.label[grep("outgroup", tree$tip.label)])} #enracine l'arbre avec l'extra-groupe
    tree <- drop.tip(tree, tip=tree$tip.label[grep("outgroup", tree$tip.label)]) #supprime l'extra-groupe
    tree <- ladderize(tree)
    ...
    plot(plant_tree)
    plant_tree$tip.label <- gsub(pattern = "_", replacement = ".", plant_tree$tip.label) #remplace les espaces par des points ds noms de plantes
    plant_tree <- ladderize(plant_tree)
    plot(plant_tree)
    
    
    network <- read.table(paste0("network_relative_incidences_",fungi,"_",marker,"_",OTU,"_",site,".csv"), sep=";", header=T)
    ...
    !plant_tree$tip.label %in% colnames(network) #verif que plantes de l'abre st ds network : true si elles n'y sont pas
    plant_tree$tip.label[!plant_tree$tip.label %in% colnames(network)] #liste des plantes qui ne st pas ds network
...
write.table(globalfit,"GlobalFit_La_Reunion.csv",sep=";",row.names=FALSE,quote=FALSE)
