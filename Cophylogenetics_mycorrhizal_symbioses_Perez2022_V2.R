...
## Approach n°1 : sample the most abundant host per symbiont species, at random if several host have the same abundance
...
for (i in 1:5) {
  ...
  for (j in 1:3){
    ...
    write.table(paste0(list_links$Var2,":", list_links$Var1), paste0("links_bifurcations_empress_", dataset, "_", fungi, "_", site, "_", seed, ".txt"), col.names=F, row.names=F, quote=F)
    fungi_tree$node.label <- NULL
    fungi_tree <- multi2di(fungi_tree)
    write.tree(fungi_tree, paste0("tree_parasites_bifurcations_empress_", dataset,"_", fungi, "_", site, "_", seed, ".tre"))
    plant_tree$node.label <- NULL
    plant_tree <- multi2di(plant_tree)
    write.tree(plant_tree, paste0("tree_hosts_bifurcations_empress_", dataset, "_", fungi, "_", site, "_", seed, ".tre"))
  }}
...
for (i in 1:5) {
  ...
  for (j in 1:3){
    ...
    reconciliation <- read.table(paste0("C:/fantine/Nextcloud/Immex_2023/data/data_",dataset,"/recon_",dataset,"_",fungi,"_",site,suffix,"_output.csv"),sep=",")
    ...
    graph <- read.table(paste0("C:/fantine/Nextcloud/Immex_2023/data/data_",dataset,"/pvalue_",dataset,"_",fungi,"_",site,suffix,"_output.svg"), comment.char = "", fill=TRUE, sep=";")
...
write.table(empress,paste0("C:/fantine/Nextcloud/Immex_2023/results/Empress_",dataset,suffix,"backbone.csv"),sep=";",row.names=FALSE,quote=FALSE)
